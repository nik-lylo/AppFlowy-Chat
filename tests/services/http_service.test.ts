...
import { ChatSettings, CreateChatParams, UpdateChatParams, CreateChatMessageParams, ChatMessage, QuestionStream, RelatedQuestion, MessageCursor, RepeatedChatMessage, ChatAuthor, ChatAuthorType, JSONValue, STREAM_ANSWER_KEY, STREAM_METADATA_KEY } from '@appflowy-chat/types/ai';
...
// create a map to store the chat id and chat object
  ...
  createChat(_workspace_id: string, params: CreateChatParams): Promise<Response<void>> {
    const settings = {
  ...
  updateChatSettings(_workspace_id: string, chat_id: string, params: UpdateChatParams): Promise<Response<void>> {
    const chatSettings = this.chatMap.get(chat_id);
    if (!chatSettings) {
      return Promise.resolve(Response.fromError(new ChatError(ErrorCode.RecordNotFound, 'Chat not found')));
    }
  ...
  getChatSettings(_workspace_id: string, chat_id: string): Promise<Response<ChatSettings | null>> {
    const chatSettings = this.chatMap.get(chat_id);
    if (!chatSettings) {
      return Promise.resolve(Response.fromError(new ChatError(ErrorCode.RecordNotFound, 'Chat not found')));
    }
  ...
  getChatMessages(_workspace_id: string, chat_id: string, cursor: MessageCursor, limit: number): Promise<Response<RepeatedChatMessage | null>> {
    ...
    switch (cursor.type) {
      ...
      case "AfterMessageId":
        ...
        startIndex = chat.messages.findIndex(msg => msg.message_id === cursor.value);
        if (startIndex === -1) {
          return Promise.resolve(Response.fromError(new ChatError(ErrorCode.RecordNotFound, 'Message not found')));
        }
      ...
      case "NextBack":
        ...
        return Promise.resolve(Response.fromError(new ChatError(ErrorCode.InvalidRequest, 'NextBack cursor type is not fully implemented')));
    }
  ...
  sendMessage(_workspace_id: string, chat_id: string, params: CreateChatMessageParams): Promise<Response<ChatMessage | null>> {
    const chat = this.chatMap.get(chat_id);
    if (!chat) {
      return Promise.resolve(Response.fromError(new ChatError(ErrorCode.RecordNotFound, 'Chat not found')));
    }
  ...
  streamMessageResponse(_workspace_id: string, chat_id: string, message_id: number): Promise<Response<QuestionStream | null>> {
    const chat = this.chatMap.get(chat_id);
    if (!chat) {
      return Promise.resolve(Response.fromError(new ChatError(ErrorCode.RecordNotFound, 'Chat not found')));
    }
  ...
  getRelatedQuestions(_workspace_id: string, _chat_id: string, _message_id: number): Promise<Response<RelatedQuestion[] | null>> {
    const questions = [{ content: 'Related question', metadata: {} }, { content: 'Related question 2', metadata: {} }, { content: 'Related question 3', metadata: {} }] as RelatedQuestion[];
    return Promise.resolve(Response.fromSuccess(questions));
  }



  private async *createStream(message_id: number): AsyncIterableIterator<JSONValue | ChatError> {
    const streamData = [
      ...
      { [STREAM_METADATA_KEY]: { meta: 'Additional metadata for message ' + message_id } },
    ];
...
describe('ChatHttpService', () => {
  ...
  test('should create a new chat', async () => {
    const params = { chat_id: 'chat1', name: 'New Chat', rag_ids: [] } as CreateChatParams;
